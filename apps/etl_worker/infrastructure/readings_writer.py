"""Insère les lectures validées dans la table structurée enervision.readings.

Distincte de readings_raw (voir postgres_writer.py), la table
d'atterrissage brute de l'ingestion temps réel : `readings` est la table
servie par core_api, peuplée par le job de transformation horaire à
partir du bucket bronze. Idempotente via la même contrainte
(site_id, timestamp).
"""
...
_INSERT_SQL = """
    INSERT INTO enervision.readings (
...
class ReadingsWriter:
    """Écrit une EnergyReading dans enervision.readings, de façon idempotente."""
