...
Worker ETL (async) - au lieu d'un déclenchement externe managé par le cloud, c'est un processus qui tourne en continu dans son propre conteneur, avec deux jobs planifiés (APScheduler interne) : ingestion des dernières lectures toutes les 60s (bucket `raw`), puis transformation horaire vers `consumption_readings` — voir [seq_etl.md](seq_etl.md). Il n'expose pas d'endpoint HTTP - ce n'est pas une API, c'est un worker.
